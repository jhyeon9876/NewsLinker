...
class KeywordExtractor:
    def __init__(self):
        ...
        def extract_names(text):
            ...
            names = [res['word'] for res in ner_results if res['entity'] == 'B-PER']
            return list(set(names))
        self.extract_names = extract_names

        def noun_and_name_extractor(text):
            results = []
            for t in text:
            ...
            names_in_text = extract_names(" ".join(text))
            results.extend(names_in_text)
            return " ".join(results)
        self.noun_and_name_extractor = noun_and_name_extractor

        def keyword_one(text, top_n=5, use_mmr=True, diversity=0.5):
    ...
    def extract_keywords(self, article, top_n=5, diversity=0.2, include_names=True):
        text = self.noun_and_name_extractor([article])
        keywords = self.keyword_one(text, top_n=top_n, use_mmr=True, diversity=diversity)
        if include_names:
            names_in_text = self.extract_names(" ".join([article]))
            keywords = [(name, 1.0) for name in names_in_text if name not in [kw[0] for kw in keywords]] + keywords[:top_n]
...
class Similarity:
    def __init__(self, extractor: KeywordExtractor, threshold=0.7):
        ...
        def cluster_consecultive_sentences(sentences, threshold=0.8):
        ...
        def get_text_by_clusters(sentences, clusters):
            clustered_text = []
            for cluster in clusters:
                cluter_text = " ".join([sentences[i] for i in cluster])
                clustered_text.append(cluter_text)
            return clustered_text
        self.get_text_by_clusters = get_text_by_clusters

    def filtering(self, text: str, keywords: list):
        sentences = self.kiwi.split_into_sents(text)
    ...
    # 문장별 키워드 추출 함수
    def extract_keywords_from_sentences(self, sentences, top_n=3):
        sentence_keywords = {}
    ...
    def process_text_by_cluster(self, text,threshold=0.8, top_n=5):
